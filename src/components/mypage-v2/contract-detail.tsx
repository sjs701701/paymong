...
export function ContractDetailView({
  ...
  const emptyUsageState = (
    <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 bg-slate-50/40 py-10 text-center">
      <ReceiptText size={24} className="text-slate-300" />
      <p className="text-sm text-slate-500">아직 이용 내역이 없어요.</p>
    </div>
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col lg:hidden">
          ...
          <div className="flex flex-col gap-3 px-4 pt-3 sm:px-6 sm:pt-5">
            ...
            <div className="overflow-clip rounded-2xl border border-slate-200 bg-white">
              ...
              {detail.usageHistory.length === 0 ? (
                <div className="mx-5 my-4">{emptyUsageState}</div>
              ) : (
                <ul className="divide-y divide-slate-100 px-5 pb-28">
                  {detail.usageHistory.map((item) => (
                    <li key={item.id}>{renderUsageItem(item)}</li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 sm:px-6 sm:pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
          <Button
            size="lg"
            onClick={onPay}
            disabled={isLimitClosed}
            className={cn(
              "pointer-events-auto h-auto w-full gap-2 rounded-xl py-4 text-base font-bold shadow-[0_18px_40px_rgba(0,56,241,0.24)]",
              isLimitClosed
